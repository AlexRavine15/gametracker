...
¡Bienvenido! Este es un proyecto de gestión de *backlog* de videojuegos desarrollado con **Node.js**, **Express** y **TypeScript**. La aplicación se conecta directamente con la API pública de **RAWG** para buscar información real de los juegos (títulos, portadas y plataformas disponibles) y permite al usuario guardar su estatus de juego y la plataforma específica donde lo está disfrutando.

El backend no solo procesa los datos de forma segura, sino que además renderiza una interfaz limpia en **modo oscuro** con un sutil sistema de estilos dinámicos que resalta visualmente la plataforma elegida utilizando bordes personalizados y efectos de elevación según la consola (PS4, PS5, Xbox Series, PC, etc.).
...
Por el momento, abre el archivo `src/index.ts` y reemplaza el valor de la constante con tu clave real:
```typescript
const RAWG_API_KEY: string = 'TU_API_KEY_REAL_AQUÍ';
```
> ⚠️ **Nota:** Próximamente se migrará a variables de entorno `.env` para evitar exponer credenciales en el código fuente.
...
1. **Migración a Variables de Envío (.env):** Ocultar credenciales sensibles usando `dotenv`.
